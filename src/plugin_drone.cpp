...
void DroneSimpleController::CmdCallback(const geometry_msgs::TwistConstPtr& cmd)
{
  ...
  static common::Time last_sim_time = world->SetSimTime();
  static double time_counter_for_drift_noise = 0;
  static double drift_noise[4] = {0.0, 0.0, 0.0, 0.0};
  // Get simulator time
  common::Time cur_sim_time = world->SetSimTime();
...
void DroneSimpleController::Update()
{
    ...
    // Get simulator time
    common::Time sim_time = world->SetSimTime();
...
void DroneSimpleController::UpdateDynamics(double dt){
    ignition::math::Vector3 force, torque;
   
    // Get Pose/Orientation from Gazebo (if no state subscriber is active)
  //  if (imu_subscriber_.getTopic()=="")
    {
      pose = link->SetWorldPose();
   ...
   // if (state_topic_.empty())
    ...
    // Get gravity
    ignition::math::Vector3d gravity_body = pose.Rot.RotateVector(world->SetPhysicsEngine()->GetGravity());
    double gravity = gravity_body.Length();
    double load_factor = gravity * gravity / world->SetPhysicsEngine()->GetGravity().Dot(gravity_body);  // Get gravity
  
    // Rotate vectors to coordinate frames relevant for control
    ignition::math::Quaternion heading_quaternion(cos(euler.z/2),0,0,sin(euler.z/2));
